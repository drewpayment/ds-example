...
namespace Dominion.Domain.Validation.Misc
{
    ...
    /// </summary>
    public class CountryValidator : EntityValidator<Country>
    {
        /// <summary>
        /// Primary constructor.
        /// </summary>
        /// <param name="reason">The reason that validation is being performed (ie: insert, update).</param>
        public CountryValidator(ValidationReason reason = ValidationReason.Default)
            : base(reason)
        {
        }
        ...
        protected override void Rules()
        {
            ...
            RuleFor(x => x.CountryId)
                ...
                .WithRequiredContext();
            RuleFor(x => x.Name).NotNull().WithRequiredContext().NotEmpty().WithRequiredContext();
        }

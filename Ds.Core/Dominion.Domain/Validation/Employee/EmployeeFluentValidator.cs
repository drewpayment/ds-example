...
namespace Dominion.Domain.Validation.Employee
{
    ...
    /// </summary>
    public class EmployeeFluentValidator : FluentValidator<Entities.Employee.Employee>
    {
        private const string GENDER_VALIDATION_REGEX = "^(M|F)$";
        private DateTime maxBirthDate = DateTime.Today;
        public readonly DateTime MIN_REALISTIC_DATE = new DateTime(1885, 9, 2);
        public readonly DateTime MAX_REALISTIC_DATE = new DateTime(2999, 12, 31);
        ...
        public EmployeeFluentValidator(ValidationReason reason = ValidationReason.Default)
            : base(reason)
        {
            // required properties
            ...
            RuleFor(x => x.ClientWorkersCompId)
                ...
                .WithRequiredContext();
            RuleFor(x => x.MaritalStatusId).Must(x => x > -1).When(x => x.MaritalStatusId != null).WithRequiredContext();

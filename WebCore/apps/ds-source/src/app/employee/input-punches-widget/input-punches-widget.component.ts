import { Component, OnInit, ChangeDetectionStrategy, ChangeDetectorRef, ViewChild, ViewChildren, QueryList, OnDestroy, Inject } from '@angular/core';
import { FormGroup, FormBuilder, Validators } from '@angular/forms';
import {
    CheckPunchTypeResult, JobCosting, ClientDivision, ClientDepartment, ClientCostCenter, ClientEarning,
    PunchTypeItem,
...
import { PunchOptionType, JobCostingType, ClientEarningCategory } from '@ds/core/employee-services/enums';
import * as moment from 'moment';
import { throwError, of, forkJoin, Observable, Observer, BehaviorSubject } from 'rxjs';
import { tap, switchMap, catchError, skipWhile, startWith, map } from 'rxjs/operators';
...
import { JobCostingIdAssignments } from '../shared/models/job-costing-aassignments.model';
...
export class InputPunchesWidgetComponent implements OnInit, OnDestroy {
    ...
    blockPunching = false;
    clockName = '';
    formControlObservers = {} as { [key: string]: BehaviorSubject<JobCostingAssignment[]> };
    ...
    formatDate(date: Moment | string, momentDateFormat: string = 'MM-DD-YYYY hh:mm a'): string {
        return moment(date).format(momentDateFormat);
    }

    private saveNormalPunch() {
        this.punchService.saveNormalPunch(this.form, this.filteredJobCostingList, this.clockName)
            .pipe(
                catchError(err => throwError(err)),
                switchMap(detail => {
                    this.dsMsg.setTemporarySuccessMessage('Punch was saved successfully!', 2500);
            ...
            .subscribe(() => this.employeeService.fetchTimeCard(this.user.clientId, this.user.employeeId),
                err => {
    ...
    private saveInputPunches() {
        this.punchService.saveInputPunches(this.form, this.filteredJobCostingList, this.clockName)
            .pipe(
                catchError(err => throwError(err)),
                switchMap(detail => {
                    this.dsMsg.setTemporarySuccessMessage('Punches were saved successfully!', 2500);
                    this.punchDetail = detail;
                    this.resetForm();
                    return of(detail);
                }),
            ...
            .subscribe(() => this.employeeService.fetchTimeCard(this.user.clientId, this.user.userEmployeeId),
                err => {
    ...
    private saveInputHours() {
        this.punchService.saveInputHours(this.form, this.filteredJobCostingList, this.clockName)
            .pipe(
                catchError(err => throwError(err)),
                switchMap(detail => {
                    this.dsMsg.setTemporarySuccessMessage('Hours saved successfully!', 2500);
                    this.punchDetail = detail;
                    this.resetForm();
                    return of(detail);
                }),
            ...
            .subscribe(detail => this.employeeService.fetchTimeCard(this.user.clientId, this.user.userEmployeeId),
                err => {
                    this.saving = false;
    ...
    private disableHiddenFields() {
        if (this.punchDetail.shouldHideCostCenter) this.form.get('costCenter').disable();
        if (this.punchDetail.shouldHideDepartment) this.form.get('department').disable();
        if (this.punchDetail.shouldHideEmployeeNotes) this.form.get('employeeNote').disable();
    ...
    private loadResources() {
        this.accountService.getUserInfo()
            ...
            .subscribe(detail => {
                ...
                if (this.punchDetail.employeeClockConfiguration &&
                    ...
                    this.rules = this.punchDetail.employeeClockConfiguration.clockEmployee.timePolicy.rules;
                ...
                if (this.punchDetail.allowInputPunches) {
                ...
                } else if (this.punchDetail.punchOption === PunchOptionType.NormalPunch) {
                    ...
                    this.service.getPunchTypeItems(this.user.userEmployeeId)
                ...
                } else if (this.punchDetail.punchOption === PunchOptionType.InputHours) {
                ...
                if (!this.showDepartment && !this.showCostCenter
                    && !this.useJobCosting && !this.showEarning) this.pageLoading = false;

                if (this.punchView == this.formTypes.hours) {
                    const ob = new BehaviorSubject<null>(null);
                    pageLoadingObservables.push(ob);
                    this.service.getEmployeeClientEarnings(this.user.selectedClientId())
                        .pipe(tap(earnings => this.earnings = earnings))
                        .subscribe(() => {
                            ob.complete();
                            this.setClientEarningFormControl();
                        });
                } else if (this.showEarning && this.punchView == this.formTypes.punches) {
                    const ob = new BehaviorSubject<null>(null);
                    pageLoadingObservables.push(ob);
                ...
                if (this.showDepartment) {
                    const ob = new BehaviorSubject<null>(null);
                    pageLoadingObservables.push(ob);
                    this.service.getEmployeeDepartments(this.user.selectedClientId(), this.user.employeeId)
                        .pipe(tap(departments => {
                            this.departments = departments;
                            this.departments.unshift({ clientDepartmentId: '-1', name: '' } as any);
                        }))
                        .subscribe(() => {
                            ob.complete();
                            this.setDepartmentFormControl();
                ...
                if (this.showCostCenter) {
                    const ob = new BehaviorSubject<null>(null);
                    pageLoadingObservables.push(ob);
                    this.service.getEmployeeCostCenters(this.user.selectedClientId())
                        .pipe(tap(costCenters => {
                            this.costCenters = costCenters;
                            if (this.punchDetail.isCostCenterSelectionRequired) {
                                this.costCenters.unshift({ clientCostCenterId: '', description: 'Select Cost Center' } as any);
                                this.form.get('costCenter').setValidators(Validators.required);
                            } else {
                                this.costCenters.unshift({ clientCostCenterId: '-1', description: '' } as any);
                            }
                        }))
                        .subscribe(() => {
                            this.setCostCenterFormControl();
                ...
                if (this.useJobCosting) {
                    ...
                    this.service.getClientDivisions(this.user.selectedClientId())
                        .subscribe(divisions => {
                            this.divisions = divisions;
                            this.divisions.unshift({ clientDivisionId: -1, name: 'Use Home Division' } as ClientDivision);
                            divisionsOb.complete();
                        });

                    this.service.getClientJobCostingList(this.user.selectedClientId()).subscribe(jobCostingList => {
                        this.jobCostingList = jobCostingList;
                        this.hiddenJobCostingList = this.jobCostingList
                            .filter(jc => jc.hideOnScreen && jc.jobCostingTypeId === JobCostingType.Employee);
                        this.filteredJobCostingList = this.jobCostingList.filter(jc => !jc.hideOnScreen);

                        if (this.hiddenJobCostingList && this.hiddenJobCostingList.length) {
    ...
    jobCostingAssignmentSelected(event: MatAutocompleteSelectedEvent, jc: JobCosting) {
        this.saving = true;
    ...
    changeJobCostingHandler(assignmentId: number, jc: JobCosting) {
        ...
        const availableChildren = this.filteredJobCostingList.filter(j => j.parentJobCostingIds.includes(jc.clientJobCostingId));
        if (availableChildren && availableChildren.length) {
            this.setJobCostingFieldsDisabledState(true);
            const idx = this.filteredJobCostingList.findIndex(j => j.clientJobCostingId == jc.clientJobCostingId);

            if (idx > -1 && jc.availableAssignments && jc.availableAssignments.length) {
                this.filteredJobCostingList[idx].jobCostingAssignmentSelection =
                    jc.availableAssignments.find(j => j.clientJobCostingAssignmentId == assignmentId);

                if (this.filteredJobCostingList[idx] && this.filteredJobCostingList[idx].jobCostingAssignmentSelection) {
                    this.filteredJobCostingList[idx].selectedAvailableAssignmentId =
                        this.filteredJobCostingList[idx].jobCostingAssignmentSelection.clientJobCostingAssignmentId;
                } else {
                    const parentFormControlName = this.resolveJobCostingFormControlName(jc);
                    this.form.controls[parentFormControlName].reset();
                }
            }

            const nextChild = availableChildren[0];
            const childFormControlName = this.resolveJobCostingFormControlName(nextChild);
            const parentJobCostingsAssignmentIdList = [];
            const parentJobCostingList = this.filteredJobCostingList
                .filter(item => nextChild.parentJobCostingIds.includes(item.clientJobCostingId));

            if (parentJobCostingList)
                parentJobCostingList.forEach(pj => parentJobCostingsAssignmentIdList.push(pj.selectedAvailableAssignmentId));
            ...
            this.service.getEmployeeJobCostingAssignmentLists(this.user.clientId, this.user.userEmployeeId, associatedJobCostingList)
                .pipe(
                    switchMap(data => of(data[0])),
                    map(data => {
                        const availableAssignments = data && data.availableAssignments ? data.availableAssignments : null;

                        nextChild.availableAssignments = availableAssignments;
                        this.formControlObservers[childFormControlName].next(availableAssignments);
                        this.cd.detectChanges();

                        if (availableAssignments && availableAssignments.length) {
                            this.updateJobCostingAssignmentListObservable(childFormControlName, availableAssignments);
                            this.manageJobCostingFormControlState(nextChild);
                        } else {
                            const nextSeq = jc.sequence + 1;
                            const queryMatOption = this.autoCompleteList
                                .find((item, index, arr) => item.id == `mat-autocomplete-${nextSeq}`);
    ...
    private updateJobCostingAssignmentListObservable(formControlName: string, value: JobCostingAssignment[]) {
    ...
    private loadHiddenJobCostingAvailableAssignments() {
        ...
        this.hiddenJobCostingList.forEach(jc => {
            ...
            this.service.getEmployeeJobCostingAssignmentLists(
                this.user.clientId, this.user.userEmployeeId, associatedJobCostingList)
                .pipe(
                    switchMap(data => of(data[0])),
                    switchMap(data => {
                        jc.availableAssignments = data.availableAssignments;
                        // set the assigned job costing for the hidden one...
                        const myEmployeeAssignment = data.availableAssignments
                            .find(aa => aa.clientJobCostingAssignmentId == this.user.userEmployeeId) || {
                                clientJobCostingAssignmentId: this.user.userEmployeeId
                            } as JobCostingAssignment;

                        jc.selectedAvailableAssignmentId = myEmployeeAssignment.clientJobCostingAssignmentId;
                        jc.jobCostingAssignmentSelection = myEmployeeAssignment;
                        this.form.get(identifier).setValue(jc.selectedAvailableAssignmentId, { emitEvent: false });

                        const jcIdx = this.jobCostingList.findIndex(i => i.clientJobCostingId == jc.clientJobCostingId);
                        if (jcIdx) {
                            this.jobCostingList[jcIdx].selectedAvailableAssignmentId =
                                data.availableAssignments[0].clientJobCostingAssignmentId;
                            this.jobCostingList[jcIdx].jobCostingAssignmentSelection = data.availableAssignments[0];
                        }
                        return of(data);
                    })
                )
    ...
    private getJobCostingAssignmentObservable(formControlName: string, jc: JobCosting): Observable<JobCostingAssignment[]> {
        return this.form.controls[formControlName].valueChanges
            .pipe(
                startWith(''),
                map(value => {
    ...
    private checkJobCostingSelectedAssignmentFormValues(parentJc: JobCosting) {
        const childJc = this.filteredJobCostingList
            .find(jcl => jcl.parentJobCostingIds.includes(parentJc.clientJobCostingId) && jcl.sequence == parentJc.sequence + 1);
        ...
        if (childSelectedValue) {
            const found = parentJc.availableAssignments.find(jca => jca.clientJobCostingAssignmentId == childSelectedValue);
            if (!found) fc.reset();
        }

        const hasGrandChild = this.filteredJobCostingList
            .find(jcl => jcl.parentJobCostingIds.includes(childJc.clientJobCostingId) && jcl.sequence == childJc.sequence + 1);
    ...
    private loadJobCostingAvailableAssignments() {
        ...
        this.filteredJobCostingList.forEach(jc => {
            ...
            if (!this.formControlObservers[formControlName]) {
                this.formControlObservers[formControlName] = new BehaviorSubject<JobCostingAssignment[]>([]);
            ...
            const parentJobCostingsAssignmentIdList = [];
            const parentJobCostingList = this.jobCostingList
                .filter(item => jc.parentJobCostingIds.includes(item.clientJobCostingId));

            if (parentJobCostingList)
                parentJobCostingList.forEach((v, i, a) =>
                    parentJobCostingsAssignmentIdList.push(v.selectedAvailableAssignmentId));
            ...
            if (!jc.hideOnScreen && jc.jobCostingTypeId != JobCostingType.Employee) {
                ...
                this.service.getEmployeeJobCostingAssignmentLists(
                    ...
                    .subscribe(data => {
                        // jc.availableAssignments = data.availableAssignments;
                        this.formControlObservers[formControlName].next(data.availableAssignments);

                        if (!jc.availableAssignments$) {
                            jc.availableAssignments$ = this.getJobCostingAssignmentObservable(formControlName, jc);
                        }

                        this.updateJobCostingAssignmentListObservable(formControlName, jc.availableAssignments);
    ...
    private manageJobCostingFormControlState(jc: JobCosting) {
        ...
        if (jc.availableAssignments && jc.availableAssignments.length && !jc.hideOnScreen) {
    ...
    private _filter(value: any, jc: JobCosting): JobCostingAssignment[] {
        ...
        return jc.availableAssignments.filter(a => a.description.toLowerCase().includes(filterValue));
    }
    ...
    private resolveJobCostingFormControlName(jc: JobCostingUIHelper): string {
        ...
        const originalJcItemIndex = this.jobCostingList.findIndex(jcl => jcl.clientJobCostingId == jc.clientJobCostingId);
    ...
    private setCostCenterFormControl() {
        ...
        const punches = this.punchDetail && this.punchDetail.employeeClockConfiguration && this.punchDetail.employeeClockConfiguration.clockEmployee
            && this.punchDetail.employeeClockConfiguration.clockEmployee
        ...
        if (!this.punchDetail.isFirstPunchOfDay && this.punchDetail.lastOutCostCenterId
            && this.punchDetail.homeCostCenterId != this.punchDetail.lastOutCostCenterId
            && (this.punchDetail.isOutPunch ||
                (!hasTwoLunchPunches &&
                    existingLunchPunches != null &&
                    existingLunchPunches.length > 0))) {
            this.form.get('costCenter').setValue(this.punchDetail.lastOutCostCenterId);
        } else if (this.punchDetail.isCostCenterSelectionRequired && this.punchDetail.homeCostCenterId == null) {
    ...
    private setPunchTypeFormControl() {
        ...
        const lunchPunchTypeId = this.punchService.qualifyLunchPunchType(this.punchDetail);
        ...
        if (this.punchDetail.punchTypeId) {
            ...
            if (defaultPunchType)
                fc.setValue(defaultPunchType.id);
            else if (lunchPunchTypeId)
                fc.setValue(lunchPunchTypeId);
        }

        if (this.punchDetail.shouldDisablePunchTypeSelection)
            fc.disable();
        else
            fc.enable();
    }
    ...
    private setClientEarningFormControl() {
        ...
        const regularEarnings = this.earnings.filter(e => e.earningCategoryId == ClientEarningCategory.Regular);

        if (regularEarnings && regularEarnings.length) {
            selectedEarning = regularEarnings.find(r => r.isDefault) || regularEarnings[0];
        }

        if (selectedEarning) this.form.controls['clientEarning'].setValue(selectedEarning.clientEarningId);
    }

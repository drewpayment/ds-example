 ...
 */
export interface DsTableDataSourcePageEvent {
  pageIndex: number;
  pageSize: number;
  length: number;
}
 ...
 * Decoupled so that users can depend on either the legacy or MDC-based paginator.
 */
export interface DsTableDataSourcePaginator {
  page: Subject<DsTableDataSourcePageEvent>;
  pageIndex: number;
...
export class _DsTableDataSource<T,
    P extends DsTableDataSourcePaginator = DsTableDataSourcePaginator>
  ...
  next(data: T[]) {
    if (this.paginator) this.paginator.length = data.length;
  ...
  _updateChangeSubscription() {
    ...
    const sortChange: Observable<Sort|null|void> = this._sort ?
        ...
        observableOf(null);
    const pageChange: Observable<DsTableDataSourcePageEvent|null|void> = this._paginator ?
        merge(
          this._paginator.page,
          this._internalPageChanges,
          this._paginator.initialized
        ) as Observable<DsTableDataSourcePageEvent|void> :
        observableOf(null);
 ...
 * own `DataSource`.
 */
export class DsTableDataSource<T> extends _DsTableDataSource<T, MatPaginator> {}
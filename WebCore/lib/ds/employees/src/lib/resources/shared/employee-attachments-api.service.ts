...
import { IEmployeeAttachmentFolderDetail } from './employee-attachment-folder-detail.model';
...
export class EmployeeAttachmentsService {
    ...
    getFolders(employeeId: number): Observable<IEmployeeAttachmentFolderDetail[]> {
        const url = `${this.api}/employees/${employeeId}/folders`;
        return this.http.get<IEmployeeAttachmentFolderDetail[]>(url)
            .pipe(
            catchError(this.httpError('getFolders', <IEmployeeAttachmentFolderDetail[]>[]))
            );
    }

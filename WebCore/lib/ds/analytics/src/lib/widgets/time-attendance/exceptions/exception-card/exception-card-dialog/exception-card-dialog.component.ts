import { Component, OnInit, Inject, ViewChild, AfterViewInit } from '@angular/core';
import { EmployeeException } from '@ds/analytics/shared/models/ExceptionData.model';
import { MatTableDataSource } from '@angular/material/table';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort, MatSortable } from '@angular/material/sort';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material/dialog';

@Component({
  selector: 'ds-exception-card-dialog',
  templateUrl: './exception-card-dialog.component.html',
  styleUrls: ['./exception-card-dialog.component.css']
})
export class ExceptionCardDialogComponent implements OnInit{
    dataSource: MatTableDataSource<EmployeeException>;
    displayedColumns: string[] = ["Date", "Name", "Department", "Schedule", "PunchTime", "Hours Effected"];

    @ViewChild(MatPaginator, { static: true }) paginator: MatPaginator;
    @ViewChild(MatSort, { static: true }) sort: MatSort;

    title: string;
    scheduleEmployeeIds: number[] = [];
    scheduleData: any[] = [];
    punchTimeTitle: string;

    constructor(
        @Inject(MAT_DIALOG_DATA) public data: any,
        public dialogRef: MatDialogRef<ExceptionCardDialogComponent>,
    ) {}

    close(): void {
        this.dialogRef.close();
    }


    ngOnInit() {
        if (this.data.employeeExceptions.length > 0){
            this.title = this.data.employeeExceptions[0].clockException;
        }
        this.getPunchException(this.title);
        this.dataSource = new MatTableDataSource<EmployeeException>(this.data.employeeExceptions);
        this.dataSource.paginator = this.paginator;
        this.sort.sort(({ id: 'eventDate', start: 'desc'}) as MatSortable); //defaults to date occured descending
        this.dataSource.sort = this.sort;
        this.dataSource.sortingDataAccessor = (item, sortColumn) => {
          switch (sortColumn) {
              case 'Name':
                  return item.fullName;
              case 'Date':
                  return item.eventDate;
              case "Schedule":
                  return item.employeeSchedule;
              case 'Department':
                  return item.department;
              case 'PunchTime':
                  return item.employeePunch;
              case 'Hours Effected':
                  return item.hours;
               default:
                  return item[this.sort.active];
          }
        };
    }

    applyFilter(filterValue: string) {
        this.dataSource.filter = filterValue.trim().toLowerCase();
    }

    print = () => {
        var employees = this.data.employeeExceptions;

        employees.sort((a, b) => { a.firstName > b.firstName ? -1 : 1 })

        var printWindow = window.open(this.data.title, 'Print' + (new Date()).getTime(), 'width=1200,height=700');
        printWindow.document.write('<!DOCTYPE html><html><head>');

        for (var i = 0; i < document.styleSheets.length; i++) {
            if (document.styleSheets[i].href) {
                printWindow.document.write('<link href= "' + document.styleSheets[i].href + '" rel="stylesheet" type="text/css" >');
            }
        }

        printWindow.document.write('</head><body style="background-color:#fff;">');
        printWindow.document.write('<style> html{font-size: 12px} @page{size:landscape;}</style>');
        printWindow.document.write('<div class="print-page">');
        printWindow.document.write('<div class="print-page-header">');
        printWindow.document.write('<h1 class="print-header">Exceptions</h1>');
        printWindow.document.write('<div class="print-page-section-group">');
        if (employees.length == 1)
        printWindow.document.write('<div class="instruction-text">'+ employees.length + ' Employee</div>');
      else{
        printWindow.document.write('<div class="instruction-text">'+ employees.length + ' Employees</div>');
      }
        printWindow.document.write('<div class="instruction-text">Date Range: ' + this.convertDate(new Date(this.data.dateRange.StartDate)) + ' - ' + this.convertDate(new Date(this.data.dateRange.EndDate)) + '</div>');
        printWindow.document.write('<div class="instruction-text">Exception Type: ' + (this.data.featureName ? this.data.featureName : 'N/A') + '</div>');
        printWindow.document.write('</div>');
        printWindow.document.write('</div>');
        printWindow.document.write('<table>');

        printWindow.document.write('<tr>');
        printWindow.document.write('<th>Name</th>');
        printWindow.document.write('<th>Department</th>');
        printWindow.document.write('<th>Date Occurred</th>');
        printWindow.document.write('<th>Hours Effected</th>');
        printWindow.document.write('</tr>');

        employees.forEach(employee => {
          printWindow.document.write('<tr>');
          printWindow.document.write('<td>' + employee.fullName + '</td>');
          printWindow.document.write('<td>' + employee.department + '</td>');
          printWindow.document.write('<td>' + this.convertDate(new Date(employee.eventDate)) + '</td>');
          printWindow.document.write('<td>' + (employee.hours ? (employee.hours.toFixed(2) + ' Hours') : '') + '</td>');
          printWindow.document.write('<tr>');
        })

        printWindow.document.write('</table>');
        printWindow.document.write('</div>')
        printWindow.document.write('<script>window.print();</script>');
        printWindow.document.write('</body></html>');
        printWindow.document.close();
        printWindow.focus();

        printWindow.addEventListener('afterprint', (event) => {
            printWindow.close();
        });
      }

      convertDate(date: Date){
        var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];

        return `${months[date.getMonth()]} ${date.getDate()}, ${date.getFullYear()}`;
      }

      getPunchException(title){
        switch(title){
          case "Arrived Early":
            this.punchTimeTitle = "Time In"
            return
          case "Tardy":
            this.punchTimeTitle = "Time In"
            return
          case "Left Early":
            this.punchTimeTitle = "Time Out"
            return
          case "Left Late":
            this.punchTimeTitle = "Time Out"
            return
          case "Under Schedule":
            this.punchTimeTitle = "Time Out"
            return
          case "Over Schedule":
            this.punchTimeTitle = "Time Out"
            return
          default:
            this.punchTimeTitle = "Exception Punch"
            return
        }
      }

      getHoursEffectedDetails(title, hours){
        if (hours){
          if (title == "Tardy" ||
          title == "Left Early" ||
          title == "Under Schedule")
          {
            return "-"+hours.toFixed(2)+" hrs"
          }

          else{
            return hours.toFixed(2)+" hrs"
          }
        }
        else{
          return ""
        }
      }
}

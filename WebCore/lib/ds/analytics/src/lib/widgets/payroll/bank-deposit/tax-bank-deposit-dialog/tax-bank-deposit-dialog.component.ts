...
@Component({
  selector: 'ds-tax-bank-deposit-dialog',
  templateUrl: './tax-bank-deposit-dialog.component.html',
  styleUrls: ['./tax-bank-deposit-dialog.component.css']
})
export class TaxBankDepositDialogComponent implements OnInit {

  dataSource: MatTableDataSource<BankDeposit>;
  displayedColumns: string[] = ['Tax', 'Payment', 'MTD', 'QTD', 'YTD'];
  title: string;
  checkDate: string;
  ...
  constructor(
    @Inject(MAT_DIALOG_DATA) public data: any, public dialogRef: MatDialogRef<TaxBankDepositDialogComponent>
  ) {}
  ...
  print = () => {
    ...
    employees.forEach(employee => {
      printWindow.document.write('<tr>');
      printWindow.document.write('<td>' + employee.taxName + '</td>');
      printWindow.document.write('<td>' + `$${employee.amount.toLocaleString('en-US')}` + '</td>');
